// Auto-generated embedded documentation for FluxScript CLI.
// Do not edit manually — regenerate from docs/FLUXSCRIPT.md
...
const char* getDocsText() {
    return R"MARKDOWN(# FluxScript Documentation
...
// Search docs for a keyword, return matching lines with context
std::string searchDocs(const std::string& keyword) {
    ...
    for (auto& [num, matchLine] : matches) {
        int start = std::max(0, num - 1 - contextBefore);
        int end = std::min((int)allLines.size(), num + contextAfter);
