...
double SweepReport::getMinOutput() const {
    ...
    for (const auto& p : data) {
        if (p.simulationSuccess && p.outputValue < min) min = p.outputValue;
    }
    return min;
}
...
double SweepReport::getMaxOutput() const {
    ...
    for (const auto& p : data) {
        if (p.simulationSuccess && p.outputValue > max) max = p.outputValue;
    }
    return max;
}
...
std::string SweepEngine::modifyNetlist(const std::string& original, double value) {
    ...
    if (pos != std::string::npos) {
        ...
        if (endLine == std::string::npos) endLine = modified.length();
        ...
        if (iss >> name >> n1 >> n2 >> oldVal) {
            ...
            if (newVal.find('.') != std::string::npos) {
                ...
                if (newVal.back() == '.') newVal.pop_back();
            }
    ...
    while (std::getline(iss, line)) {
        if (line.find("Vin") != std::string::npos && line.find("DC") != std::string::npos) {
            ...
            if (eq != std::string::npos) {
                try { vin = std::stod(line.substr(eq + 1)); } catch(...) {}
            }
        }
    }
...
SweepReport SweepEngine::run() {
    ...
    std::string originalNetlist((std::istreambuf_iterator<char>(file)),
