...
REPL::REPL() : m_running(false), m_scope(0) {
    // Initialize keywords
    m_keywords = {"let", "fun", "if", "else", "for", "in", "return", "import", 
                  "module", "export", "true", "false", "pi", "e"};
    
    // Initialize built-in functions
    m_functions = {"sin", "cos", "tan", "exp", "log", "log10", "sqrt", "abs",
                   "floor", "ceil", "round", "min", "max", "sum", "mean", "plot",
                   "print", "len", "range", "append", "help", "type"};
    
    // Initialize documentation
    m_documentation = {
        {"sin", "sin(x) - Sine function (radians)"},
        {"cos", "cos(x) - Cosine function (radians)"},
        {"tan", "tan(x) - Tangent function (radians)"},
        {"exp", "exp(x) - Exponential function e^x"},
        {"log", "log(x) - Natural logarithm"},
        {"sqrt", "sqrt(x) - Square root"},
        {"plot", "plot(x, y) - Create a plot"},
        {"print", "print(x) - Print value to console"},
        {"let", "let x = value - Define a variable"},
        {"fun", "fun name(args) { body } - Define a function"}
    };
}
...
void REPL::run() {
    ...
    while (m_running) {
        ...
        if (m_config.multiLine && !continuing) {
            int openParens = std::count(input.begin(), input.end(), '(') - 
                            std::count(input.begin(), input.end(), ')');
            int openBraces = std::count(input.begin(), input.end(), '{') - 
                            std::count(input.begin(), input.end(), '}');
        ...
        if (continuing) {
            buffer += "\n" + input;
            int openParens = std::count(buffer.begin(), buffer.end(), '(') - 
                            std::count(buffer.begin(), buffer.end(), ')');
            int openBraces = std::count(buffer.begin(), buffer.end(), '{') - 
                            std::count(buffer.begin(), buffer.end(), '}');
