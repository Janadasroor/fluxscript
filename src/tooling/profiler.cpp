...
Profiler::ScopedTimer::ScopedTimer(const std::string& name, const std::string& file, int line)
    : m_name(name), m_file(file), m_line(line), m_start(std::chrono::steady_clock::now())
{
...
void Profiler::functionExit(const std::string& name)
{
    ...
    auto totalDuration = std::chrono::duration_cast<std::chrono::nanoseconds>(endTime - entry.startTime);
...
ProfileReport Profiler::getReport() const
{
    ...
    report.totalDuration =
        std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now() - m_startTime);
    report.totalCalls = 0;
    ...
    std::sort(report.entries.begin(), report.entries.end(),
              [](const ProfileEntry& a, const ProfileEntry& b) { return a.totalTime > b.totalTime; });
...
void Profiler::saveReport(const std::string& filename, int topN)
{
    ...
    file << std::left << std::setw(40) << "Function" << std::right << std::setw(12) << "Calls" << std::setw(15)
         << "Total (ms)" << std::setw(15) << "Self (ms)" << std::setw(10) << "Avg (s)"
         << "\n";
    file << std::string(92, '-') << "\n";
    ...
    for (const auto& entry : report.entries) {
        ...
        double avgUs = (entry.callCount > 0) ? (totalMs * 1000.0 / entry.callCount) : 0;

        file << std::left << std::setw(40) << entry.functionName << std::right << std::setw(12) << entry.callCount
             << std::setw(15) << std::fixed << std::setprecision(2) << totalMs << std::setw(15) << std::fixed
             << std::setprecision(2) << selfMs << std::setw(10) << std::fixed << std::setprecision(2) << avgUs << "\n";
    }
...
void Profiler::printMemoryReport()
{
    ...
    if (!leaks.empty()) {
        ...
        for (const auto& leak : leaks) {
            std::cout << "  " << leak.size << " bytes at " << leak.address << " (" << leak.file << ":" << leak.line
                      << ")\n";
        }
    }
}
...
void ProfileReport::print(int topN) const
{
    ...
    std::cout << std::left << std::setw(40) << "Function" << std::right << std::setw(12) << "Calls" << std::setw(15)
              << "Total (ms)" << std::setw(15) << "Self (ms)" << std::setw(10) << "Avg (s)"
              << "\n";
    ...
    for (const auto& entry : entries) {
        ...
        std::cout << std::left << std::setw(40) << entry.functionName << std::right << std::setw(12) << entry.callCount
                  << std::setw(15) << std::fixed << std::setprecision(2) << totalMs << std::setw(15) << std::fixed
                  << std::setprecision(2) << selfMs << std::setw(10) << std::fixed << std::setprecision(2) << avgUs
                  << "\n";
    }
...
std::string ProfileReport::toFlameGraph() const
{
    ...
    svg << "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"1200\" height=\"" << (100 + entries.size() * 30)
        << "\">\n";
    svg << "<style>rect{stroke:#000;stroke-width:1}text{font-family:monospace;font-size:14}</style>\n";
    svg << "<rect x=\"0\" y=\"0\" width=\"1200\" height=\"" << (100 + entries.size() * 30) << "\" fill=\"#f0f0f0\"/>\n";
    ...
    for (const auto& entry : entries) {
        ...
        double selfPct = (entry.totalTime.count() > 0) ? (double)entry.selfTime.count() / entry.totalTime.count() : 0;
        int r = 200 + (int)(55 * (1 - selfPct));
        int g = 100 + (int)(100 * selfPct);
        int b = 50;

        svg << "<rect x=\"50\" y=\"" << y << "\" width=\"" << width << "\" height=\"25\" fill=\"rgb(" << r << "," << g
            << "," << b << ")\"/>\n";
        svg << "<text x=\"60\" y=\"" << (y + 18) << "\">" << entry.functionName << " ("
            << (entry.totalTime.count() / 1e6) << "ms, " << entry.callCount << " calls)</text>\n";
