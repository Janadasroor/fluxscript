...
std::string CircuitDiffResult::toText() const
{
    ...
    oss << "Changes:\n";
    oss << "  Components: +" << addedComponents << " -" << removedComponents << " ~" << modifiedComponents << "\n";
    oss << "  Nets:       +" << addedNets << " -" << removedNets << " ~" << modifiedNets << "\n";
        ...
        for (const auto& diff : componentDiffs) {
            ...
            case ChangeType::Modified:
                ...
                for (const auto& prop : diff.changedProperties) {
                    oss << "    " << prop << ": " << diff.oldValue.at(prop) << " -> " << diff.newValue.at(prop) << "\n";
                }
                break;
            default:
                break;
    ...
    if (!performanceImpacts.empty()) {
        ...
        for (const auto& impact : performanceImpacts) {
            oss << "  " << impact.metric << ": " << impact.oldValue << " -> " << impact.newValue << impact.unit << " ("
...
CircuitDiffResult CircuitDiffEngine::diff(const std::string& oldFile, const std::string& newFile)
{
    ...
    std::string oldCircuit((std::istreambuf_iterator<char>(oldStream)), std::istreambuf_iterator<char>());
    std::string newCircuit((std::istreambuf_iterator<char>(newStream)), std::istreambuf_iterator<char>());
...
CircuitDiffResult CircuitDiffEngine::diffStrings(const std::string& oldCircuit, const std::string& newCircuit)
{
    CircuitDiffResult result;
    ...
    result.totalChanges = result.addedComponents + result.removedComponents + result.modifiedComponents +
                          result.addedNets + result.removedNets + result.modifiedNets;
...
CircuitDiffResult CircuitDiffEngine::diffAST(const std::string& oldAST, const std::string& newAST)
{
...
std::vector<std::string> CircuitDiffEngine::generateRecommendations(const CircuitDiffResult& result)
{
    ...
    for (const auto& diff : result.componentDiffs) {
        if (diff.change == ChangeType::Modified) {
            for (const auto& prop : diff.changedProperties) {
                if (prop == "value") {
                    recommendations.push_back("Verify that " + diff.componentName + " value change is intentional");
                }
            }
        }
    }
...
std::vector<NetDiff> CircuitDiffEngine::diffNets(const std::string& oldCircuit, const std::string& newCircuit)
{
...
std::map<std::string, std::map<std::string, std::string>> CircuitDiffEngine::parseComponents(const std::string& circuit)
{
    ...
    while (std::getline(stream, line)) {
        ...
        if (lineStream >> name >> node1 >> node2 >> value) {
            ...
            components[name] = {{"type", type}, {"node1", node1}, {"node2", node2}, {"value", value}};
        }
    }
...
CircuitDiffResult circuit_diff(const std::string& oldFile, const std::string& newFile, const DiffOptions& options)
{
...
const char* flux_diff_strings(void* diff, const char* oldCircuit, const char* newCircuit)
{
    static std::string result;
    auto* engine = static_cast<CircuitDiffEngine*>(diff);
    auto diffResult = engine->diffStrings(oldCircuit ? oldCircuit : "", newCircuit ? newCircuit : "");
    result = diffResult.toText();
    return result.c_str();
}
