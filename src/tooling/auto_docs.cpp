...
std::string Documentation::toMarkdown() const
{
    ...
    if (config.includeBOM && !bom.empty()) {
        ...
        for (const auto& entry : bom) {
            oss << "| " << entry.designator << " | " << entry.type << " | " << entry.value << " | " << entry.tolerance
                << " | " << entry.package << " | " << entry.manufacturer << " | " << entry.partNumber << " |\n";
        }
        oss << "\n";
    }
...
std::string Documentation::toHTML() const
{
    ...
    if (config.includeBOM && !bom.empty()) {
        ...
        for (const auto& entry : bom) {
            oss << "<tr><td>" << entry.designator << "</td><td>" << entry.type << "</td><td>" << entry.value
                << "</td><td>" << entry.tolerance << "</td><td>" << entry.package << "</td><td>" << entry.manufacturer
...
std::string Documentation::toText() const
{
    ...
    if (config.includeBOM && !bom.empty()) {
        ...
        oss << "-----------------\n";
        for (const auto& entry : bom) {
            oss << "  " << entry.designator << ": " << entry.type << " " << entry.value << " (" << entry.package
                << ")\n";
        }
        oss << "\n";
    }
    ...
    while (std::getline(stream, line)) {
        ...
        if (lineStream >> name >> node1 >> node2 >> value) {
            ...
            case 'R':
                ...
                break;
            case 'C':
                entry.type = "Capacitor";
                break;
            case 'L':
                entry.type = "Inductor";
                break;
            case 'Q':
                entry.type = "Transistor";
                break;
            case 'U':
                entry.type = "IC";
                break;
            case 'D':
                ...
                break;
            case 'V':
                entry.type = "Voltage Source";
                break;
            case 'I':
                entry.type = "Current Source";
                break;
            default:
                entry.type = "Unknown";
                break;
...
std::string DocGenerator::inferCircuitType(const std::string& content)
{
    if (content.find("filter") != std::string::npos)
        return "filter";
    if (content.find("amp") != std::string::npos)
        return "amplifier";
    if (content.find("osc") != std::string::npos)
        return "oscillator";
    return "general";
}
...
std::string DocGenerator::generateDescription(const std::string& circuitType)
{
    if (circuitType == "filter") {
        return "This circuit is an analog filter designed to attenuate unwanted frequency components while passing "
               "desired signals.";
    } else if (circuitType == "amplifier") {
        return "This circuit is an analog amplifier designed to increase signal amplitude while maintaining signal "
               "integrity.";
