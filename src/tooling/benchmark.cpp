...
void BenchmarkRunner::registerTest(const std::string& name, const std::string& language, const std::string& category,
                                   BenchmarkFunc func, BenchmarkFunc validation)
{
    TestInfo info;
    info.name = name;
...
BenchmarkResult BenchmarkRunner::analyzeResults(const std::string& name, const std::string& language,
                                                const std::string& category, const std::vector<double>& times,
                                                int iterations)
{
    ...
    result.medianTime = (sorted.size() % 2 == 0) ? (sorted[mid - 1] + sorted[mid]) / 2.0 : sorted[mid];
...
BenchmarkResult BenchmarkRunner::runTest(const std::string& name, int iterations, int warmupIterations)
{
    // Find test
    ...
    if (m_verbose) {
        std::cout << std::fixed << std::setprecision(3) << result.meanTime << " ms (" << result.iterations
                  << " iterations)" << std::endl;
    }
...
#else
    suite.compilerInfo = "GCC " + std::to_string(__GNUC__) + "." + std::to_string(__GNUC_MINOR__);
#endif
...
BenchmarkSuiteResult BenchmarkRunner::runByCategory(const std::string& category, int iterations)
{
...
BenchmarkSuiteResult BenchmarkRunner::runByLanguage(const std::string& language, int iterations)
{
    std::vector<std::string> testNames;
    for (const auto& test : m_tests) {
...
double BenchmarkRunner::calculateSpeedup(const BenchmarkResult& baseline, const BenchmarkResult& comparison)
{
    if (baseline.meanTime == 0)
        return 1.0;
    return baseline.meanTime / comparison.meanTime;
}

std::string BenchmarkRunner::generateComparisonTable(const std::vector<BenchmarkResult>& results)
{
    ...
    for (const auto& pair : byTest) {
        ...
        for (const auto& r : pair.second) {
            if (r.language.find("Python") != std::string::npos && r.language.find("NumPy") == std::string::npos) {
        ...
        for (const auto& r : pair.second) {
            ...
            oss << " " << std::left << std::setw(23) << r.name << " " << std::left << std::setw(16) << r.language << " "
                << std::right << std::setw(10) << std::fixed << std::setprecision(3) << r.meanTime << " " << std::right
...
void BenchmarkSuiteResult::printSummary() const
{
    ...
    std::cout << std::left << std::setw(25) << "Language" << std::right << std::setw(15) << "Avg Speedup" << "\n";
    std::cout << std::string(40, '-') << "\n";

    for (const auto& pair : avgSpeedup) {
        double avg = pair.second / count[pair.first];
        std::cout << std::left << std::setw(25) << pair.first << std::right << std::setw(14) << std::fixed
...
void BenchmarkSuiteResult::saveCSV(const std::string& filename) const
{
    ...
    // Data
    for (const auto& r : results) {
        file << r.name << "," << r.language << "," << r.category << "," << std::fixed << std::setprecision(3)
             << r.meanTime << "," << std::fixed << std::setprecision(3) << r.stdDev << "," << std::fixed
             << std::setprecision(3) << r.minTime << "," << std::fixed << std::setprecision(3) << r.maxTime << ","
             << std::fixed << std::setprecision(3) << r.medianTime << "," << r.iterations << "," << std::fixed
             << std::setprecision(0) << r.opsPerSecond << "," << std::fixed << std::setprecision(2) << r.speedup << ","
             << (r.passed ? "1" : "0") << "\n";
    }
