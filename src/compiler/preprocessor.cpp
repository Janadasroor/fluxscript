...
DirectiveType Preprocessor::parseDirective(const std::string& line, std::string& argument)
{
    ...
    if (name == "ifdef")
        return DirectiveType::Ifdef;
    if (name == "ifndef")
        return DirectiveType::Ifndef;
    if (name == "if")
        return DirectiveType::If;
    if (name == "elif")
        return DirectiveType::Elif;
    if (name == "else")
...
bool Preprocessor::evaluateCondition(const std::string& expr)
{
    ...
    while (std::regex_search(processed, match, definedRegex)) {
        ...
        processed = processed.substr(0, match.position()) + (isDef ? "1" : "0") +
...
std::string Preprocessor::handleInclude(const std::string& args, PreprocessResult& result)
{
    ...
    if (!includedResult.success) {
        result.errors.insert(result.errors.end(), includedResult.errors.begin(), includedResult.errors.end());
    }
    ...
    while (std::getline(inputStream, line)) {
        ...
        if (directive != DirectiveType::None) {
            ...
            case DirectiveType::Define:
                if (!m_skipMode)
                    handleDefine(directiveArg);
                break;
            case DirectiveType::Undef:
                if (!m_skipMode)
                    handleUndef(directiveArg);
                break;
