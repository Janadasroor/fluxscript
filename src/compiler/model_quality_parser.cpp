...
std::unique_ptr<ExprAST> Parser::ParseAssertDecl()
{
    ...
    if (CurTok == static_cast<int>(TokenType::tok_identifier) && m_lexer.IdentifierStr == "within") {
    ...
    // Optional: message
    if (CurTok == static_cast<int>(TokenType::tok_identifier) && m_lexer.IdentifierStr == "message") {
    ...
    std::cout << "[Parser] Assert: " << quantity << "(" << node << ") " << op << " " << bound << std::endl;
...
std::unique_ptr<ExprAST> Parser::ParseSettleDecl()
{
    ...
    // Parse after time
    if (CurTok != static_cast<int>(TokenType::tok_identifier) || m_lexer.IdentifierStr != "after") {
    ...
    std::cout << "[Parser] Settle: V(" << node << ") within " << tol << "% after " << afterTime << "s" << std::endl;
...
std::unique_ptr<ExprAST> Parser::ParseGoldenDecl()
{
    ...
    std::string filename;
    if (CurTok == static_cast<int>(TokenType::tok_identifier) && m_lexer.IdentifierStr == "from") {
...
std::unique_ptr<ExprAST> Parser::ParseCompareDecl()
{
    ...
    if (CurTok != static_cast<int>(TokenType::tok_identifier) || m_lexer.IdentifierStr != "with") {
        ReportError("expected 'with' keyword");
        return nullptr;
    }
    getNextToken(); // eat with

    if (CurTok != static_cast<int>(TokenType::tok_identifier) || m_lexer.IdentifierStr != "golden") {
    ...
    if (CurTok == static_cast<int>(TokenType::tok_identifier) && m_lexer.IdentifierStr == "tolerance") {
    ...
    std::cout << "[Parser] Compare: V(" << node << ") with golden '" << goldenName << "' tol=" << tolerance << "%"
              << std::endl;
...
std::unique_ptr<ExprAST> Parser::ParseConvergeDecl()
{
    ...
    int maxIter = 100;
    if (CurTok == static_cast<int>(TokenType::tok_identifier) && m_lexer.IdentifierStr == "max") {
    ...
    // Parse epsilon
    double epsilon = 1e-6;
    if (CurTok == static_cast<int>(TokenType::tok_identifier) && m_lexer.IdentifierStr == "epsilon") {
    ...
    std::cout << "[Parser] Converge: V(" << node << ") max_iter=" << maxIter << " eps=" << epsilon << std::endl;
...
std::unique_ptr<ExprAST> Parser::ParseDiscontinuityDecl()
{
    ...
    std::cout << "[Parser] Discontinuity: V(" << node << ") threshold=" << threshold << std::endl;
