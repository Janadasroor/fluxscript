...
std::string NetlistGenerator::generateNetlist(const std::vector<std::unique_ptr<FunctionAST>>& functions,
...
std::string NetlistGenerator::generateSubcktCard(const SubcktAST* subckt)
{
    ...
    for (const auto& stmt : subckt->getBody()) {
        if (auto* bsource = dynamic_cast<const BSourceExprAST*>(stmt.get())) {
            oss << "B" << bsource->getName() << " " << bsource->getPositiveNode() << " " << bsource->getNegativeNode()
                << " " << (bsource->isCurrentSource() ? "I" : "V") << "={"
                << expressionToSpice(bsource->getExpression()) << "}\n";
        } else if (auto* esource = dynamic_cast<const ESourceExprAST*>(stmt.get())) {
            oss << "E" << esource->getName() << " " << esource->getPositiveNode() << " " << esource->getNegativeNode()
                << " " << esource->getControlPosNode() << " " << esource->getControlNegNode() << " "
                << expressionToSpice(esource->getGain()) << "\n";
        } else if (auto* fsource = dynamic_cast<const FSourceExprAST*>(stmt.get())) {
            oss << "F" << fsource->getName() << " " << fsource->getPositiveNode() << " " << fsource->getNegativeNode()
                << " " << fsource->getVoltageSourceName() << " " << expressionToSpice(fsource->getGain()) << "\n";
        } else if (auto* gsource = dynamic_cast<const GSourceExprAST*>(stmt.get())) {
            oss << "G" << gsource->getName() << " " << gsource->getPositiveNode() << " " << gsource->getNegativeNode()
                << " " << gsource->getControlPosNode() << " " << gsource->getControlNegNode() << " "
                << expressionToSpice(gsource->getTransconductance()) << "\n";
        } else if (auto* hsource = dynamic_cast<const HSourceExprAST*>(stmt.get())) {
            oss << "H" << hsource->getName() << " " << hsource->getPositiveNode() << " " << hsource->getNegativeNode()
                << " " << hsource->getVoltageSourceName() << " " << expressionToSpice(hsource->getTransresistance())
                << "\n";
            ...
            for (const auto& node : inst->getNodes())
                oss << " " << node;
            oss << " " << inst->getSubcktName();
            for (const auto& [name, val] : inst->getParameters())
                oss << " " << name << "=" << val;
            oss << "\n";
...
std::string NetlistGenerator::generateResistor(const std::string& name, const std::string& n1, const std::string& n2,
                                               double value)
{
    return "R" + name + " " + n1 + " " + n2 + " " + valueToSpice(value) + "\n";
}

std::string NetlistGenerator::generateCapacitor(const std::string& name, const std::string& n1, const std::string& n2,
                                                double value)
{
    return "C" + name + " " + n1 + " " + n2 + " " + valueToSpice(value) + "\n";
}

std::string NetlistGenerator::generateInductor(const std::string& name, const std::string& n1, const std::string& n2,
                                               double value)
{
    return "L" + name + " " + n1 + " " + n2 + " " + valueToSpice(value) + "\n";
}

std::string NetlistGenerator::generateDiode(const std::string& name, const std::string& n1, const std::string& n2,
                                            const std::string& model)
{
    return "D" + name + " " + n1 + " " + n2 + " " + model + "\n";
}

std::string NetlistGenerator::generateBJT(const std::string& name, const std::string& nc, const std::string& nb,
                                          const std::string& ne, const std::string& ns, const std::string& model)
{
    return "Q" + name + " " + nc + " " + nb + " " + ne + " " + ns + " " + model + "\n";
}

std::string NetlistGenerator::generateMOSFET(const std::string& name, const std::string& nd, const std::string& ng,
                                             const std::string& ns, const std::string& nb, const std::string& model)
{
    return "M" + name + " " + nd + " " + ng + " " + ns + " " + nb + " " + model + "\n";
}

std::string NetlistGenerator::generateBSource(const std::string& name, const std::vector<std::string>& nodes,
                                              const std::string& type, const std::string& expression)
{
    if (nodes.size() < 2)
        return "";

    std::ostringstream oss;
    oss << "B" << name << " " << nodes[0] << " " << nodes[1] << " " << type << "={" << expression << "}\n";
    return oss.str();
}

std::string NetlistGenerator::generateBVoltage(const std::string& name, const std::string& nPlus,
                                               const std::string& nMinus, const std::string& expression)
{
    return generateBSource(name, {nPlus, nMinus}, "V", expression);
}

std::string NetlistGenerator::generateBCurrent(const std::string& name, const std::string& nPlus,
                                               const std::string& nMinus, const std::string& expression)
{
...
std::string NetlistGenerator::generateESource(const std::string& name, const std::string& nPlus,
                                              const std::string& nMinus, const std::string& cnPlus,
                                              const std::string& cnMinus, double gain)
{
    std::ostringstream oss;
    oss << "E" << name << " " << nPlus << " " << nMinus << " " << cnPlus << " " << cnMinus << " " << gain << "\n";
    return oss.str();
}

// F-source (Current-Controlled Current Source)
std::string NetlistGenerator::generateFSource(const std::string& name, const std::string& nPlus,
                                              const std::string& nMinus, const std::string& vSourceName, double gain)
{
    std::ostringstream oss;
    oss << "F" << name << " " << nPlus << " " << nMinus << " " << vSourceName << " " << gain << "\n";
    return oss.str();
}

// G-source (Voltage-Controlled Current Source)
std::string NetlistGenerator::generateGSource(const std::string& name, const std::string& nPlus,
                                              const std::string& nMinus, const std::string& cnPlus,
                                              const std::string& cnMinus, double gain)
{
    std::ostringstream oss;
    oss << "G" << name << " " << nPlus << " " << nMinus << " " << cnPlus << " " << cnMinus << " " << gain << "\n";
    return oss.str();
}

// H-source (Current-Controlled Voltage Source)
std::string NetlistGenerator::generateHSource(const std::string& name, const std::string& nPlus,
                                              const std::string& nMinus, const std::string& vSourceName, double gain)
{
    std::ostringstream oss;
    oss << "H" << name << " " << nPlus << " " << nMinus << " " << vSourceName << " " << gain << "\n";
    return oss.str();
}
...
std::string NetlistGenerator::generateADevice(const std::string& name, ADeviceType deviceType,
...
std::string NetlistGenerator::generateWaveFile(const std::string& name, const std::string& nPlus,
                                               const std::string& nMinus, const std::string& filePath, int channel,
                                               bool isCurrent)
{
    std::ostringstream oss;
    char prefix = isCurrent ? 'I' : 'V';
    oss << prefix << name << " " << nPlus << " " << nMinus << " WAVEFILE \"" << filePath << "\" CHAN " << channel
        << "\n";
    return oss.str();
}

std::string NetlistGenerator::generateSubcktInstance(const std::string& name, const std::string& subcktName,
...
std::string NetlistGenerator::expressionToSpice(const ExprAST* expr)
{
    ...
    if (auto* bin = dynamic_cast<const BinaryExprAST*>(expr)) {
        return "(" + expressionToSpice(bin->getLHS()) + " " + Lexer::tokenSpelling(bin->getOp()) + " " +
               expressionToSpice(bin->getRHS()) + ")";
    }

    if (auto* un = dynamic_cast<const UnaryExprAST*>(expr)) {
        return "(" + Lexer::tokenSpelling(un->getOp()) + expressionToSpice(un->getOperand()) + ")";
    }
