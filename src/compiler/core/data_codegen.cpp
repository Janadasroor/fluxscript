...
TypedValue IndexExprAST::codegen(CodegenContext& context)
{
    ...
    if (!ArrayTV.Val) {
        logError(context, "Index array expression failed to codegen", this);
        return TypedValue();
    }
    ...
    if (ArrayTV.Type.Kind == TypeKind::Vector) {
        ...
        if (!IdxTV.Val) {
            logError(context, "Vector index expression failed to codegen", this);
            return TypedValue();
        }
    ...
    if (IsMatrixIndex) {
        ...
        if (hasRowRange || hasColRange) {
            ...
            if (!codegenRange(RowIndex, RowsVal, r0, r1)) {
                logError(context, "Matrix slice row range failed to codegen", this);
                return TypedValue();
            }
            if (!codegenRange(ColIndex ? ColIndex : RowIndex, ColsVal, c0, c1)) {
                logError(context, "Matrix slice column range failed to codegen", this);
                return TypedValue();
            }
        ...
        if (!RowTV.Val || !ColTV.Val) {
            logError(context, "Matrix element index expression failed to codegen", this);
            return TypedValue();
        }
