...
TypedValue IfExprAST::codegen(CodegenContext& context)
{
    ...
    if (!CondTV.Val) {
        logError(context, "If condition sub-expression failed to codegen", this);
        return TypedValue();
    }
    ...
    if (!ThenTV.Val && ThenTV.Type.Kind != TypeKind::Void) {
        logError(context, "If-then branch failed to codegen", this);
        return TypedValue();
    }
    ...
    if (!ElseTV.Val && ElseTV.Type.Kind != TypeKind::Void) {
        logError(context, "If-else branch failed to codegen", this);
        return TypedValue();
    }
...
TypedValue ForExprAST::codegen(CodegenContext& context)
{
    ...
    if (!StartTV.Val || !EndTV.Val || !StepTV.Val) {
        logError(context, "For-loop range sub-expression failed to codegen", this);
        return TypedValue();
    }
    ...
    if (!BodyTV.Val) {
        logError(context, "For-loop body failed to codegen", this);
        return TypedValue();
    }
...
TypedValue WhileExprAST::codegen(CodegenContext& context)
{
    ...
    TypedValue CondTV = Cond->codegen(context);
    if (!CondTV.Val) {
        logError(context, "While condition failed to codegen", this);
        return TypedValue();
    }
    llvm::Value* CondV = boolCondition(CondTV.Val, context.Builder, context.TheContext);
    ...
    TypedValue BodyTV = Body->codegen(context);
    if (!BodyTV.Val) {
        logError(context, "While body failed to codegen", this);
        return TypedValue();
    }
    if (context.DebugEnabled)
        context.LexicalBlocks.pop_back();
