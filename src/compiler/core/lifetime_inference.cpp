...
void LifetimeInference::collectFromExpr(const ExprAST* expr, const std::set<std::string>& lifetimeParamNames)
{
    (void)expr;
    (void)lifetimeParamNames;
}

void LifetimeInference::collectFromType(const FluxType& type, const std::string& containingLifetime,
                                        const std::set<std::string>& lifetimeParamNames)
{
    if (type.isRef()) {
        ...
        if (innerType.isRef()) {
            ...
            if (!innerLt.empty() && innerLt[0] == '\'')
                innerLt = innerLt.substr(1);
...
bool LifetimeInference::checkConsistency(const std::set<std::string>& lifetimeParamNames,
                                         ...
                                         const FluxType& returnType)
{
    ...
    std::function<bool(const std::string&)> dfs = [&](const std::string& node) -> bool {
        ...
        for (auto& next : outlives[node]) {
            if (visited[next] == 1) {
                m_errors.push_back("[FLUX ERROR] Cycle in lifetime constraints: '" + node + "' and '" + next +
