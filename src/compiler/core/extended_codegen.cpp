...
TypedValue TryCatchExprAST::codegen(CodegenContext& context)
{
    ...
    if (!LastThrownF)
        LastThrownF =
            llvm::Function::Create(llvm::FunctionType::get(DoubleTy, {}, false), llvm::Function::ExternalLinkage,
...
TypedValue TryPropagateExprAST::codegen(CodegenContext& context)
{
    ...
    if (!InnerTV.Val) {
        logError(context, "`?` inner expression failed to codegen", this);
        return TypedValue();
    }
    ...
    if (!InnerTy->isStructTy()) {
        logError(context, "`?` requires a tagged-union (Result/Option); got non-struct type", this);
        return InnerTV; // fall through, best-effort
    }

    llvm::StructType* STy = llvm::cast<llvm::StructType>(InnerTy);
    if (STy->getNumElements() < 2) {
        logError(context, "`?` requires a tagged-union with tag + payload", this);
        return InnerTV;
    }
...
TypedValue ThrowExprAST::codegen(CodegenContext& context)
{
    ...
    if (!ExTV.Val) {
        logError(context, "throw expression sub-expression failed to codegen", this);
        return TypedValue();
    }
...
TypedValue AssertExprAST::codegen(CodegenContext& context)
{
    ...
    if (!CondTV.Val) {
        logError(context, "assert condition failed to codegen", this);
        return TypedValue();
    }
    ...
    if (!ExitF) {
        ExitF = llvm::Function::Create(llvm::FunctionType::get(llvm::Type::getVoidTy(context.TheContext),
                                                               {llvm::Type::getInt32Ty(context.TheContext)}, false),
...
TypedValue YieldExprAST::codegen(CodegenContext& context)
{
    if (!context.GeneratorStateAlloca) {
        logError(context, "yield used outside of a generator function.", this);
        return TypedValue();
    }
    ...
    if (!ValueTV.Val) {
        logError(context, "yield value expression failed to codegen", this);
        return TypedValue();
    }
...
TypedValue AwaitExprAST::codegen(CodegenContext& context)
{
    if (!context.AsyncStateAlloca) {
        logError(context, "await used outside of an async function.", this);
        return TypedValue();
    }
    ...
    TypedValue ValueTV = Value->codegen(context);
    if (!ValueTV.Val) {
        logError(context, "await expression failed to codegen", this);
        return TypedValue();
    }
    ...
    if (!ResumeTV.Val) {
        logError(context, "await resume expression failed to codegen", this);
        return TypedValue();
    }
    ...
    llvm::Value* LoadedVal =
        context.Builder.CreateLoad(llvm::Type::getDoubleTy(context.TheContext), context.AsyncResultAlloca, "await_val");
...
TypedValue CornerExprAST::codegen(CodegenContext& context)
{
    ...
    for (size_t i = 0; i < NumCases; i++) {
        ...
        if (!CaseTV.Val) {
            logError(context, "corner case expression failed to codegen", this);
            return TypedValue();
        }
...
static bool extractEnumVariant(ExprAST* pattern, CodegenContext& context, std::string& enumName,
...
TypedValue MatchExprAST::codegen(CodegenContext& context)
{
    ...
    if (!MatchValTV.Val) {
        logError(context, "match value expression failed to codegen", this);
        return TypedValue();
    }
    ...
    if (MatchValTV.Val->getType()->isPointerTy()) {
        ...
        if (!loadedTy)
            loadedTy = llvm::Type::getDoubleTy(context.TheContext);
        ...
        if (enumTypeId >= 0 && enumTypeId < static_cast<int>(context.EnumTypes.size())) {
            ...
            for (auto const& variant : enumInfo.Variants) {
                if (matchedVariants.find(variant) == matchedVariants.end()) {
                    logError(context, "match on enum '" + enumInfo.Name + "' does not cover variant '" + variant + "'", this);
                    return TypedValue();
                }
            }
        }
    ...
    for (size_t i = 0; i < Arms.size(); ++i) {
        ...
        for (size_t p = 0; p < Arms[i].first.size(); ++p) {
            armCheckBBs.push_back(llvm::BasicBlock::Create(
                Ctx, "match_check_" + std::to_string(i) + "_" + std::to_string(p), TheFunction));
        }
    ...
    if (checkBBs.empty()) {
        ...
        if (DefaultArm) {
            ...
            if (!DefaultTV.Val) {
                logError(context, "match default arm (no-arms path) failed to codegen", this);
                return TypedValue();
            }
        ...
        if (!armBindings.empty() || !armNamedBindings.empty()) {
            if (isPayloadEnum) {
                ...
                if (matchedVarIdx >= 0) {
                    ...
                    if (concretePayloadType.Kind == TypeKind::UserStruct && concretePayloadType.StructTypeId < 0 &&
                        !concretePayloadType.GenericName.empty()) {
                    ...
                    if (concretePayloadType.Kind == TypeKind::UserEnum && concretePayloadType.EnumTypeId < 0 &&
                        !concretePayloadType.GenericName.empty()) {
                    ...
                    if (!armNamedBindings.empty() && concretePayloadLLVMTy->isStructTy() &&
                        ...
                        for (const auto& [fieldName, varName] : armNamedBindings) {
                            ...
                            if (fieldIdx >= 0) {
                                llvm::Type* fieldTy =
                                    concretePayloadLLVMTy->getStructElementType(static_cast<unsigned>(fieldIdx));
                                allSharedAllocas[i][varName] =
                                    context.Builder.CreateAlloca(fieldTy, nullptr, varName + "_alloca");
                        ...
                        for (size_t bi = 0; bi < armBindings.size(); ++bi) {
                            ...
                            allSharedAllocas[i][armBindings[bi]] =
                                context.Builder.CreateAlloca(fieldTy, nullptr, armBindings[bi] + "_alloca");
                ...
                allSharedAllocas[i][binderName] =
                    context.Builder.CreateAlloca(MatchValTV.Val->getType(), nullptr, binderName + "_alloca");
    ...
    for (size_t i = 0; i < Arms.size(); ++i) {
        ...
        for (size_t p = 0; p < Patterns.size(); ++p) {
            ...
            if (isPayloadEnum) {
                ...
                if (!PatDiscVal) {
                    ...
                    if (!PatternTV.Val) {
                        logError(context, "match pattern (payload enum) failed to codegen", this);
                        return TypedValue();
                    }
                ...
                if (PatDiscVal->getType() != llvm::Type::getInt32Ty(Ctx))
                    PatDiscVal =
                        context.Builder.CreateIntCast(PatDiscVal, llvm::Type::getInt32Ty(Ctx), false, "pat_tag_casted");
                IsMatch = context.Builder.CreateICmpEQ(TagVal, PatDiscVal,
                                                       "tag_cmp_" + std::to_string(i) + "_" + std::to_string(p));
            } else if (isEnumMatch) {
                TypedValue PatternTV = Pattern->codegen(context);
                if (!PatternTV.Val) {
                    logError(context, "match pattern (simple enum) failed to codegen", this);
                    return TypedValue();
                }
                ...
                IsMatch = context.Builder.CreateICmpEQ(matchVal, patVal,
                ...
                if (!binderName.empty()) {
                    ...
                    TypedValue PatternTV = Pattern->codegen(context);
                    if (!PatternTV.Val) {
                        logError(context, "match pattern (non-enum) failed to codegen", this);
                        return TypedValue();
                    }
                    ...
                    if (MatchVal->getType()->isIntegerTy()) {
                        if (PatternVal->getType() != MatchVal->getType())
                            PatternVal =
                                context.Builder.CreateIntCast(PatternVal, MatchVal->getType(), false, "pat_cast");
                        IsMatch = context.Builder.CreateICmpEQ(MatchVal, PatternVal,
                                                               "cmp_" + std::to_string(i) + "_" + std::to_string(p));
                        ...
                        IsMatch = context.Builder.CreateFCmpOEQ(MatchVal, PatternVal,
                                                                "cmp_" + std::to_string(i) + "_" + std::to_string(p));
                    } else {
            ...
            if ((!armBindings.empty() || !armNamedBindings.empty()) && isPayloadEnum) {
                llvm::BasicBlock* ExtractBB = llvm::BasicBlock::Create(
                    Ctx, "match_extract_" + std::to_string(i) + "_" + std::to_string(p), TheFunction);
                ...
                llvm::Value* matchValAlloca =
                    context.Builder.CreateAlloca(MatchValTV.Val->getType(), nullptr, "match_val_alloc");
                context.Builder.CreateStore(MatchValTV.Val, matchValAlloca);
                llvm::Value* payloadPtr =
                    context.Builder.CreateStructGEP(MatchValTV.Val->getType(), matchValAlloca, 1, "payload_ptr");
                ...
                if (matchedVarIdx >= 0) {
                    ...
                    bool isBoxed = matchedVarIdx < static_cast<int>(enumInfo.VariantIsBoxed.size())
                                       ? enumInfo.VariantIsBoxed[matchedVarIdx]
                                       : false;
                    ...
                    if (!armNamedBindings.empty() && concretePayloadLLVMTy->isStructTy() &&
                        ...
                        for (const auto& [fieldName, varName] : armNamedBindings) {
                            ...
                            if (fieldIdx >= 0) {
                                llvm::Value* fieldPtr =
                                    context.Builder.CreateStructGEP(concretePayloadLLVMTy, concretePayloadPtr,
                        ...
                        for (size_t bi = 0; bi < armBindings.size(); ++bi) {
                            llvm::Value* fieldPtr = context.Builder.CreateStructGEP(
                                concretePayloadLLVMTy, concretePayloadPtr, bi, armBindings[bi]);
                            llvm::Type* fieldTy = concretePayloadLLVMTy->getStructElementType(bi);
                            llvm::Value* FieldVal =
                                context.Builder.CreateLoad(fieldTy, fieldPtr, armBindings[bi] + "_val");
        ...
        if (!ResultTV.Val) {
            logError(context, "match arm result expression failed to codegen", this);
            return TypedValue();
        }
    ...
    if (DefaultArm) {
        ...
        if (!DefaultBB) {
            ...
            context.Builder.SetInsertPoint(checkBBs.empty() ? context.Builder.GetInsertBlock()
                                                            : checkBBs.back().back());
        } else {
        ...
        TypedValue DefaultTV = DefaultArm->codegen(context);
        if (!DefaultTV.Val) {
            logError(context, "match default arm result expression failed to codegen", this);
            return TypedValue();
        }
        llvm::Value* DefaultV = DefaultTV.Val;
...
TypedValue ForeachExprAST::codegen(CodegenContext& context)
{
    ...
    if (!IterableTV.Val) {
        logError(context, "foreach iterable expression failed to codegen", this);
        return TypedValue();
    }
...
TypedValue RepeatUntilExprAST::codegen(CodegenContext& context)
{
    ...
    TypedValue BodyTV = Body->codegen(context);
    if (!BodyTV.Val) {
        logError(context, "repeat-until body expression failed to codegen", this);
        return TypedValue();
    }
    ...
    TypedValue CondTV = Condition->codegen(context);
    if (!CondTV.Val) {
        logError(context, "repeat-until condition expression failed to codegen", this);
        return TypedValue();
    }
...
TypedValue DoWhileExprAST::codegen(CodegenContext& context)
{
    ...
    TypedValue BodyTV = Body->codegen(context);
    if (!BodyTV.Val) {
        logError(context, "do-while body expression failed to codegen", this);
        return TypedValue();
    }
    context.Builder.CreateBr(CondBB);
    ...
    if (!CondTV.Val) {
        logError(context, "do-while condition expression failed to codegen", this);
        return TypedValue();
    }
    ...
    if (CondTV.Val->getType()->isIntegerTy(1)) {
        ...
        CondBool = context.Builder.CreateFCmpONE(CondTV.Val, llvm::ConstantFP::get(llvm::Type::getDoubleTy(Ctx), 0.0),
                                                 "dowhile_check");
    }
