...
bool ModuleLoader::parseModuleFile(const std::filesystem::path& path, ModuleInfo& info, std::string* error)
{
    ...
    while (std::getline(file, line)) {
        ...
        if (std::regex_search(line, match, nameRegex)) {
            ...
            info.version = Version(std::stoi(match[1]), std::stoi(match[2]), std::stoi(match[3]));
...
bool ModuleLoader::compileModule(const std::filesystem::path& sourcePath, const std::filesystem::path& outputPath,
                                 std::string* error)
{
    // Read source file
    ...
    if (!ifs.is_open()) {
        if (error)
            *error = "Cannot open source file: " + sourcePath.string();
        return false;
    }
    ...
    if (!mod) {
        if (error)
            *error = "No LLVM module after compilation";
        return false;
    }
    ...
    std::cout << "[ModuleLoader] Compiled: " << sourcePath.string() << " -> " << outputPath.string() << std::endl;
    return true;
}

bool ModuleLoader::loadCompiledModule(const std::filesystem::path& path, ModuleData& data, std::string* error)
{
...
bool ModuleLoader::loadImport(const ImportSpec& spec, std::string* error)
{
    ...
    if (spec.minVersion.major > 0 && info.version < spec.minVersion) {
        if (error) {
            *error = "Module version too old: " + spec.moduleName + " (required >= " + spec.minVersion.toString() +
    ...
    if (spec.maxVersion.major > 0 && spec.maxVersion < info.version) {
        if (error) {
            *error = "Module version too new: " + spec.moduleName + " (required <= " + spec.maxVersion.toString() +
                     ", found " + info.version.toString() + ")";
        }
        return false;
    }
...
{
    return m_loadedModules.find(moduleName) != m_loadedModules.end() && m_loadedModules.at(moduleName).isLoaded;
}
...
void ModuleLoader::loadStandardLibrary(std::string* error)
{
    ...
    std::vector<std::string> stdlibNames = {"stdlib", "flux_stdlib", "flux-stdlib"};
