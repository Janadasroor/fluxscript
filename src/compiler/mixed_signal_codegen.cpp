...
TypedValue FSMExprAST::codegen(CodegenContext& context) {
    ...
    if (!addTransFunc) {
        ...
        llvm::Type* CondFnPtrTy = llvm::PointerType::get(CondFnTy, 0);
        llvm::Type* OutFnPtrTy = llvm::PointerType::get(CondFnTy, 0);
...
TypedValue TriggeredExprAST::codegen(CodegenContext& context) {
    ...
    if (!ThenResult) ThenResult = llvm::ConstantFP::get(context.TheContext, llvm::APFloat(0.0));
