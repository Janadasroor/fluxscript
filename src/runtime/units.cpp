...
ScaledUnit UnitRegistry::parseUnitString(const std::string& str) const {
    ...
    if (slashPos != std::string::npos) {
        ...
        return ScaledUnit(
            numUnit.dimensions / denUnit.dimensions,
            numUnit.scale / denUnit.scale,
            numStr + "/" + denStr,
            str
        );
    }
    ...
    if (powerPos != std::string::npos) {
        ...
        return ScaledUnit(
            baseUnit.dimensions.pow(power),
            std::pow(baseUnit.scale, power),
            baseUnit.name + "^" + std::to_string(power),
            str
        );
    }
    ...
    if (str.find("²") != std::string::npos) {
        ...
        ScaledUnit baseUnit = parseUnitString(baseStr);
        return ScaledUnit(
            baseUnit.dimensions.pow(2),
            std::pow(baseUnit.scale, 2),
            baseUnit.name + "^2",
            str
        );
    }
    if (str.find("³") != std::string::npos) {
        ...
        ScaledUnit baseUnit = parseUnitString(baseStr);
        return ScaledUnit(
            baseUnit.dimensions.pow(3),
            std::pow(baseUnit.scale, 3),
            baseUnit.name + "^3",
            str
        );
    }
...
std::string UnitRegistry::getCanonicalUnit(const UnitDimensions& dims) const {
    ...
    if (dims == Dimension::Power) return "W";
    if (dims == Dimension::Energy) return "J";
    if (dims == Dimension::Charge) return "C";
    if (dims == Dimension::Time) return "s";
    if (dims == Dimension::Length) return "m";
    if (dims == Dimension::Mass) return "kg";
    if (dims == Dimension::Temperature) return "K";
    if (dims == Dimension::Force) return "N";
    if (dims == Dimension::Pressure) return "Pa";
