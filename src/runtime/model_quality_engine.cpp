...
bool ModelQualityEngine::assertVoltage(const std::string& node, const std::string& op, 
    ...
    for (const auto& point : history) {
        if (withinTime > 0 && point.time > withinTime) continue;
        
        bool conditionMet = false;
        if (op == "<") conditionMet = point.value < bound;
        else if (op == ">") conditionMet = point.value > bound;
        else if (op == "<=") conditionMet = point.value <= bound;
        else if (op == ">=") conditionMet = point.value >= bound;
        else if (op == "==") conditionMet = std::abs(point.value - bound) < m_absTol;
        
        if (!conditionMet) {
            passed = false;
            std::ostringstream oss;
            oss << "V(" << node << ")=" << point.value << "V at t=" << point.time 
                << "s violates " << op << " " << bound << "V";
            failMsg = oss.str();
            break;
        }
    }
...
double ModelQualityEngine::checkSettling(const std::string& node, double tolPercent, double afterTime) {
    ...
    for (size_t i = 0; i < history.size(); ++i) {
        if (history[i].time < afterTime) continue;
...
bool ModelQualityEngine::compareWaveform(const std::string& node, const std::string& goldenName, 
                                         double tolPercent) {
...
bool ModelQualityEngine::detectHiddenState(const std::string& node, int historyDepth) {
    auto& history = m_voltageHistory[node];
    if ((int)history.size() < historyDepth * 2) return false;
    ...
    result.message = foundPattern ? 
        "Hidden state detected (pattern length=" + std::to_string(patternLength) + ")" :
