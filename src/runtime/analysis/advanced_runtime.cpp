                        ...
                        const std::vector<std::vector<double>>& yData)
{
    ...
    if (!xData.empty()) {
        std::cout << " X Range: " << std::setw(20) << xData.front() << " to " << std::setw(20) << xData.back() << "  "
                  << std::endl;
    }
...
OptimizationResult flux_run_optimization(const OptimizationConfig& config)
{
    ...
    for (int iter = 0; iter < config.maxIterations; ++iter) {
        ...
        if (reportInterval > 0 && (iter + 1) % reportInterval == 0) {
            ...
            std::cout << "  Progress: " << std::setw(3) << percent << "% | Best fitness: " << std::setw(10)
...
std::vector<BenchmarkResult> flux_run_benchmark(const BenchmarkConfig& config)
{
    ...
    for (const auto& tool : config.comparators) {
        ...
        std::cout << "  " << std::left << std::setw(15) << tool << ": " << std::fixed << std::setprecision(1)
    ...
    for (const auto& r : results) {
        std::cout << " " << std::left << std::setw(14) << r.tool << " " << std::right << std::setw(7) << std::fixed
                  << std::setprecision(1) << r.simulationTimeMs << "  " << std::setw(7) << r.memoryUsageMB << "  "
    ...
    if (results.size() > 1) {
        ...
        for (size_t i = 1; i < results.size(); ++i) {
            ...
            std::cout << "  " << results[0].tool << " is " << std::fixed << std::setprecision(1) << speedup
                      << "x faster than " << results[i].tool << std::endl;
        }
    }
...
int flux_benchmark_export_csv(const std::vector<BenchmarkResult>& results, const std::string& filename)
{
    ...
    for (const auto& r : results) {
        file << r.tool << "," << r.simulationTimeMs << "," << r.memoryUsageMB << "," << r.accuracyPercent << ","
             << r.startupTimeMs << "\n";
    }
...
int flux_sweep_init(const SweepConfig& config)
{
    ...
    for (const auto& ctrl : config.controls) {
        ...
        if (ctrl.type == "slider" || ctrl.type == "knob") {
            ctrlStr += " [" + std::to_string(static_cast<int>(ctrl.minValue)) + " to " +
                       std::to_string(static_cast<int>(ctrl.maxValue)) + "]";
        }
...
int flux_generate_report(const ReportConfig& config)
{
    ...
    if (config.includePNG)
        std::cout << "    PNG Images                                         " << std::endl;
    if (config.includeCSV)
        std::cout << "    CSV Data                                            " << std::endl;
    if (config.includeNetlist)
        std::cout << "    SPICE Netlist                                     " << std::endl;
    if (config.includeBOM)
        std::cout << "    Bill of Materials                                   " << std::endl;
...
{
    std::cout << "Added table to " << section << " (" << headers.size() << " columns, " << data.size() << " rows)"
              << std::endl;
    return 0;
}
