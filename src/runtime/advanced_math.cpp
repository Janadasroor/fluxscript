...
double flux_covariance(double* x, double* y, int n) {
    ...
    double cov = 0;
    for (int i = 0; i < n; i++) cov += (x[i] - mx) * (y[i] - my);
    return cov / (n - 1);
}
...
double flux_percentile(double* data, int n, double p) {
    ...
    double frac = idx - lo;
    if (lo >= n - 1) return g_buf1.back();
    return g_buf1[lo] + frac * (g_buf1[lo + 1] - g_buf1[lo]);
}

double flux_skewness(double* data, int n) {
    if (!data || n < 3) return 0.0;
    double sum = 0; for (int i = 0; i < n; i++) sum += data[i];
    double m = sum / n;
    double var = 0; for (int i = 0; i < n; i++) var += (data[i]-m)*(data[i]-m);
    double s = std::sqrt(var / (n - 1));
    if (s == 0) return 0.0;
    double skew = 0; for (int i = 0; i < n; i++) skew += std::pow((data[i]-m)/s, 3);
    return skew / n;
}
...
double flux_kurtosis(double* data, int n) {
    ...
    var /= (n - 1);
    if (var == 0) return 0.0;
    double fourth = 0; for (int i = 0; i < n; i++) fourth += std::pow((data[i]-m), 4);
    fourth /= n;
    return fourth / (var * var) - 3.0;
}

double flux_median_filter(double* data, int n, int window) {
    if (!data || n < 1 || window < 1) return 0.0;
    g_buf1.assign(data, data + n);
    for (int i = 0; i < n; i++) {
        g_buf2.clear();
...
double* flux_interp1_linear(double* xq, int nq, double* xp, double* yp, int np) {
    if (!xq || !xp || !yp || nq < 1 || np < 2) return nullptr;
    g_buf1.resize(nq);
    for (int i = 0; i < nq; i++) {
        double x = xq[i];
        if (x <= xp[0]) { g_buf1[i] = yp[0]; continue; }
        if (x >= xp[np-1]) { g_buf1[i] = yp[np-1]; continue; }
...
double* flux_interp1_spline(double* xq, int nq, double* xp, double* yp, int np) {
    ...
    for (int i = 0; i < n; i++) h[i] = xp[i+1] - xp[i];
    for (int i = 1; i < n; i++)
    ...
    for (int i = 0; i < nq; i++) {
        double x = xq[i];
        if (x <= xp[0]) { g_buf1[i] = yp[0]; continue; }
        if (x >= xp[n]) { g_buf1[i] = yp[n]; continue; }
        int j = std::upper_bound(xp, xp + np, x) - xp - 1;
        double dx = x - xp[j];
...
double flux_erf(double x) { return std::erf(x); }
double flux_erfc(double x) { return std::erfc(x); }
...
#else
double flux_bessel_j0(double x) { (void)x; return 0.0; }
double flux_bessel_j1(double x) { (void)x; return 0.0; }
...
double* flux_conv(double* x, int nx, double* y, int ny) {
    if (!x || !y || nx < 1 || ny < 1) return nullptr;
    int n = nx + ny - 1;
    g_buf1.assign(n, 0.0);
    for (int i = 0; i < nx; i++)
        for (int j = 0; j < ny; j++)
...
double* flux_polyfit(double* x, double* y, int n, int order) {
    if (!x || !y || n < 2 || order < 1 || order >= n) return nullptr;
    int m = order + 1;
    g_buf1.resize(m * n);
    g_buf2.resize(n);
    g_buf3.resize(m * m);
    ...
    for (int col = 0; col < m - 1; col++) {
        ...
        for (int row = col + 1; row < m; row++)
            if (std::abs(a[row + col * m]) > std::abs(a[pivot + col * m]))
                pivot = row;
        if (pivot != col) {
            for (int j = col; j < m; j++) std::swap(a[col + j * m], a[pivot + j * m]);
            std::swap(b[col], b[pivot]);
        }
        double piv_val = a[col + col * m];
        if (std::abs(piv_val) < 1e-15) continue;
        for (int row = col + 1; row < m; row++) {
            double factor = a[row + col * m] / piv_val;
            for (int j = col; j < m; j++) a[row + j * m] -= factor * a[col + j * m];
            b[row] -= factor * b[col];
        }
    }
    g_buf4.assign(m, 0.0);
    for (int i = m - 1; i >= 0; i--) {
        double sum = b[i];
        for (int j = i + 1; j < m; j++) sum -= a[i + j * m] * g_buf4[j];
...
double* flux_polyroots(double* coeffs, int order) {
    if (!coeffs || order < 1) return nullptr;
    if (std::abs(coeffs[0]) < 1e-15) return nullptr;
    ...
    auto poly = [&](double x) -> double {
        double r = 0.0;
        for (int i = 0; i <= order; i++) r = r * x + coeffs[i];
        return r;
    };
    auto deriv = [&](double x) -> double {
        double r = 0.0;
        for (int i = 0; i < order; i++) r = r * x + coeffs[i] * (order - i);
        return r;
    };
    for (int k = 0; k < order; k++) {
        ...
        for (int iter = 0; iter < 100; iter++) {
            ...
            if (std::abs(p) < 1e-12) break;
            if (std::abs(dp) < 1e-15) { x += 0.1; continue; }
            x -= p / dp;
        }
        g_buf1[k * 2] = x;
        g_buf1[k * 2 + 1] = 0;
...
double* flux_least_squares(double* a, double* b, int m, int n) {
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++) {
            double sum = 0;
            for (int k = 0; k < m; k++) sum += a[k + i * m] * a[k + j * m];
            ata[i + j * n] = sum;
        }
    vector<double> atb(n);
    for (int i = 0; i < n; i++) {
        double sum = 0;
        for (int k = 0; k < m; k++) sum += a[k + i * m] * b[k];
        atb[i] = sum;
    }
    ...
    for (int col = 0; col < n - 1; col++) {
        ...
        for (int row = col + 1; row < n; row++)
            if (std::abs(ata[row + col * n]) > piv) { piv = std::abs(ata[row + col * n]); pivot = row; }
        if (pivot != col) {
            for (int j = col; j < n; j++) std::swap(ata[col + j * n], ata[pivot + j * n]);
        ...
        for (int row = col + 1; row < n; row++) {
            ...
            for (int j = col; j < n; j++) ata[row + j * n] -= factor * ata[col + j * n];
            atb[row] -= factor * atb[col];
        }
    }
    for (int i = n - 1; i >= 0; i--) {
        double sum = atb[i];
        for (int j = i + 1; j < n; j++) sum -= ata[i + j * n] * g_buf1[j];
...
static void* new_mat(int r, int c) { return flux_register_new_matrix(r, c); }
...
void* flux_matrix_qr(void* m) {
    auto* M = mat(m);
    if (!M) return nullptr;
    auto* result = new Eigen::MatrixXd(M->householderQr().householderQ() *
...
double flux_matrix_norm(void* m, double type) {
    ...
    switch (t) {
        case 0: return M->lpNorm<Eigen::Infinity>();
        case 1: return M->lpNorm<1>();
        case 2: return M->norm();
        default: return M->operatorNorm();
    }
}
