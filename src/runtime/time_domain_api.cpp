...
BSourceOutput TimeDomainEngine::evaluateBSource(const std::string& name, double time, const NodeVoltages& inputs)
{
...
double flux_evaluate_bsource(const char* name, double time, double timestep, const double* nodeVoltages, int numNodes)
{
    if (!name)
        return 0.0;
