...
std::shared_ptr<SymbolicExpr> SymbolicExpr::makePower(std::shared_ptr<SymbolicExpr> base,
                                                      std::shared_ptr<SymbolicExpr> exp)
{
...
std::shared_ptr<SymbolicExpr> SymbolicEngine::power(std::shared_ptr<SymbolicExpr> base,
                                                    std::shared_ptr<SymbolicExpr> exp)
{
...
std::shared_ptr<SymbolicExpr> SymbolicEngine::simplify(std::shared_ptr<SymbolicExpr> expr)
{
    ...
    if (simplified->type == SymbolicExpr::Add || simplified->type == SymbolicExpr::Mul) {
        if (simplified->children.size() == 2) {
            ...
            if (a->type == SymbolicExpr::Number && a->value == 0 && simplified->type == SymbolicExpr::Add)
                return b;
            if (b->type == SymbolicExpr::Number && b->value == 0 && simplified->type == SymbolicExpr::Add)
                return a;
            if (a->type == SymbolicExpr::Number && a->value == 1 && simplified->type == SymbolicExpr::Mul)
                return b;
            if (b->type == SymbolicExpr::Number && b->value == 1 && simplified->type == SymbolicExpr::Mul)
                return a;
            if (a->type == SymbolicExpr::Number && a->value == 0 && simplified->type == SymbolicExpr::Mul)
                return a; // Corrected: 0*x = 0
            if (b->type == SymbolicExpr::Number && b->value == 0 && simplified->type == SymbolicExpr::Mul)
                return b; // Corrected: x*0 = 0
        }
    }
...
std::shared_ptr<SymbolicExpr> SymbolicEngine::differentiate(std::shared_ptr<SymbolicExpr> expr, const std::string& var)
{
    ...
    case SymbolicExpr::Symbol:
        if (expr->name == var)
            result = SymbolicExpr::makeNumber(1);
        else
    ...
    case SymbolicExpr::Add:
        result = SymbolicExpr::makeAdd(differentiate(expr->children[0], var), differentiate(expr->children[1], var));
        break;
    ...
    case SymbolicExpr::Power: {
        ...
        if (exp->type == SymbolicExpr::Number) {
            ...
            result = SymbolicExpr::makeMul(SymbolicExpr::makeNumber(n),
...
std::shared_ptr<SymbolicExpr> SymbolicEngine::laplace(std::shared_ptr<SymbolicExpr> expr, const std::string& t,
                                                      const std::string& s)
{
    return registerExpr(expr);
}

std::shared_ptr<SymbolicExpr> SymbolicEngine::inverseLaplace(std::shared_ptr<SymbolicExpr> expr, const std::string& s,
                                                             const std::string& t)
{
    return registerExpr(expr);
}
...
SymbolicEngine::jacobian(const std::vector<std::shared_ptr<SymbolicExpr>>& exprs, const std::vector<std::string>& vars)
{
