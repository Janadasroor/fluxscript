...
void BuckConverter::selectComponents()
{
    ...
    std::cout << "Specs: Vin=" << m_specs.Vin << "V, Vout=" << m_specs.Vout << "V, Iout=" << m_specs.Iout
              << "A, fsw=" << (m_specs.fsw / 1e6) << "MHz\n\n";
...
PowerSimulationResult BuckConverter::simulate()
{
    ...
    double Psw = 0.5 * m_specs.Vin * m_specs.Iout * (m_components.Qg * m_specs.fsw / 0.5); // Gate drive loss
    ...
    // Ripple calculations
    double dI = m_specs.Iout * m_specs.rippleCurrent;
    result.vout_ripple = dI * m_components.Cout_ESR + dI / (8 * m_specs.fsw * m_components.Cout);
...
std::string BuckConverter::toNetlist() const
{
    ...
    oss << "* Vin=" << m_specs.Vin << "V, Vout=" << m_specs.Vout << "V, Iout=" << m_specs.Iout << "A\n\n";
...
bool BuckConverter::validate() const
{
    ...
    if (m_components.L <= 0 || m_components.Cout <= 0)
        return false;
    if (m_components.R1 <= 0 || m_components.R2 <= 0)
        return false;
...
std::shared_ptr<BuckConverter> PowerSupplyFactory::createBuck(double Vin, double Vout, double Iout, double fsw)
{
