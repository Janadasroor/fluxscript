...
TransferFunction TransferFunction::feedback(const TransferFunction& H) const {
    ...
    // Add feedback
    if (den.size() < num.size()) den.resize(num.size(), 0);
...
NyquistData NyquistAnalyzer::analyze(const TransferFunction& tf,
...
std::string TransferFunction::toString() const {
    ...
    for (size_t i = 0; i < m_num.size(); ++i) {
        oss << m_num[i];
        if (i < m_num.size() - 1) oss << ", ";
    }
    oss << "], Den: [";
    for (size_t i = 0; i < m_den.size(); ++i) {
        oss << m_den[i];
        if (i < m_den.size() - 1) oss << ", ";
    }
    oss << "]";
    return oss.str();
}
...
FrequencyResponse BodeAnalyzer::analyze(const TransferFunction& tf,
                                         double fStart, double fStop,
                                         int pointsPerDecade)
{
...
OpAmpStabilityAnalyzer::OpAmpStabilityAnalyzer() 
    : m_closedLoopGain(1), m_compensated(false) {}
