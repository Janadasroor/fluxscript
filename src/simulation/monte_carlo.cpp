...
MonteCarloEngine::MonteCarloEngine()
    : m_numRuns(1000), m_numThreads(4), m_currentRun(0), m_convergenceThreshold(0.01), m_converged(false),
...
void MonteCarloEngine::addVariation(const std::string& name, double nominal, double tolerance, DistributionType dist)
{
...
double MonteCarloEngine::sampleDistribution(const ComponentVariation& var)
{
    ...
    case DistributionType::TruncatedNormal: {
        ...
        do {
            value = normal(m_rng);
        } while (value < var.nominal * (1 - 3 * var.tolerance) || value > var.nominal * (1 + 3 * var.tolerance));
        return value;
    }
...
MCStatistics MonteCarloEngine::getStatistics() const
{
    ...
    for (auto& pair : measurementValues) {
        ...
        // Median
        size_t mid = values.size() / 2;
        ms.median = (values.size() % 2 == 0) ? (values[mid - 1] + values[mid]) / 2 : values[mid];
...
double MonteCarloEngine::getCp() const
{
    ...
    double tolerance = (spec.hasUpper && spec.hasLower) ? (spec.upper - spec.lower) / 2 : ms.std_dev * 6;
...
void MonteCarloEngine::saveResults(const std::string& filename)
{
    ...
    for (size_t i = 0; i < m_results.size(); ++i) {
        ...
        file << (result.converged ? "1" : "0") << "," << result.executionTime << "\n";
    }
...
MCStatistics monte_carlo_simulate(const std::vector<ComponentVariation>& variations,
                                  const std::vector<SpecLimits>& specs, MonteCarloEngine::MeasurementFunction func,
                                  int numRuns, unsigned int seed)
{
...
void flux_mc_add_variation(void* mc, const char* name, double nominal, double tolerance, int dist_type)
{
...
void flux_mc_add_spec(void* mc, const char* measurement, double lower, double upper, int has_lower, int has_upper)
{
    auto* engine = static_cast<MonteCarloEngine*>(mc);
    SpecLimits spec;
