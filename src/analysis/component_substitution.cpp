...
std::vector<ComponentSpec> ComponentDatabase::search(const std::string& query)
{
    ...
    for (const auto& pair : m_components) {
        ...
        if (spec.partNumber.find(query) != std::string::npos || spec.manufacturer.find(query) != std::string::npos ||
...
void ComponentDatabase::exportToCSV(const std::string& filename)
{
    ...
    for (const auto& pair : m_components) {
        const auto& spec = pair.second;
        file << spec.partNumber << "," << spec.type << "," << spec.value << "," << spec.tolerance << "," << spec.package
             << "," << spec.manufacturer << "," << spec.price << "," << spec.availability << "\n";
    }
...
SubstitutionEngine::SubstitutionEngine() : m_minAvailability(100), m_maxPriceIncrease(0.5) {}

SubstitutionEngine::~SubstitutionEngine() = default;

std::vector<SubstitutionSuggestion>
SubstitutionEngine::findSubstitutes(const ComponentSpec& original,
                                    const std::vector<std::string>& preferredManufacturers)
{
    ...
    for (const auto& candidate : candidates) {
        if (candidate.partNumber == original.partNumber)
            continue;

        // Check availability
        if (candidate.availability < m_minAvailability)
            continue;
        ...
        if (!preferredManufacturers.empty()) {
            auto it = std::find(preferredManufacturers.begin(), preferredManufacturers.end(), candidate.manufacturer);
        ...
        if (candidate.tolerance != original.tolerance) {
            suggestion.differences.push_back("Tolerance: " + candidate.tolerance + " vs " + original.tolerance);
        }
        if (candidate.package != original.package) {
            suggestion.differences.push_back("Package: " + candidate.package + " vs " + original.package);
...
std::vector<SubstitutionSuggestion> SubstitutionEngine::findSubstitutesByPartNumber(const std::string& partNumber)
{
    auto& db = ComponentDatabase::instance();
...
SubstitutionEngine::BOMSubstitutionResult SubstitutionEngine::substituteBOM(const std::vector<ComponentSpec>& bom)
{
...
bool SubstitutionEngine::isCompatible(const ComponentSpec& original, const ComponentSpec& substitute)
{
    return calculateCompatibilityScore(original, substitute) >= 0.7;
}

double SubstitutionEngine::calculateCompatibilityScore(const ComponentSpec& original, const ComponentSpec& substitute)
{
    double score = 1.0;
...
std::vector<SubstitutionSuggestion>
SubstitutionEngine::rankSuggestions(std::vector<SubstitutionSuggestion>& suggestions)
{
...
void SubstitutionEngine::checkParametricCompatibility(const ComponentSpec& original, const ComponentSpec& substitute,
                                                      SubstitutionSuggestion& suggestion)
{
...
const char* flux_sub_find(void* engine, const char* partNumber)
{
    static std::string result;
    auto suggestions =
        static_cast<SubstitutionEngine*>(engine)->findSubstitutesByPartNumber(partNumber ? partNumber : "");
...
const char* flux_bom_analyze(void* analyzer)
{
    ...
    result = "BOM Analysis: " + std::to_string(analysis.totalComponents) + " components, $" +
             std::to_string(analysis.totalCost) + " total";
    return result.c_str();
}
}
