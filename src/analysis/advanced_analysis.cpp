...
std::string StabilityResult::toString() const
{
    ...
    oss << "\n\n";
    oss << "Phase Margin:      " << phaseMargin << " at " << phaseMarginFreq / 1000 << " kHz\n";
    oss << "Gain Margin:       " << gainMargin << " dB at " << gainMarginFreq / 1000 << " kHz\n";
    oss << "Bandwidth (-3dB):  " << bandwidth / 1000 << " kHz\n";
    oss << "Peak Gain:         " << peakGain << " dB at " << peakGainFreq / 1000 << " kHz\n";
...
std::string SensitivityResult::toString() const
{
    ...
    for (const auto& comp : sensitivities) {
        oss << comp.name << " | " << comp.sensitivity << " | " << comp.percentChange << "% | " << comp.criticality
            << "\n";
    }
...
std::string SensitivityResult::toMarkdown() const
{
    ...
    oss << "|-----------|-------------|----------|-------------|\n";
    for (const auto& comp : sensitivities) {
        oss << "| " << comp.name << " | " << comp.sensitivity << " | " << comp.percentChange << "% | "
            << comp.criticality << " |\n";
    }
    return oss.str();
}
...
std::string OptimizationResult::toString() const
{
    ...
    for (const auto& pair : optimalValues) {
        ...
        oss << pair.first << " | " << initial << " | " << pair.second << " | "
...
double SensitivityAnalyzer::calculateSensitivity(const std::string& netlist, const std::string& paramName,
                                                 double nominalValue, double deltaPercent, const std::string& outputVar)
{
...
SensitivityResult SensitivityAnalyzer::analyze(const std::string& circuitFile,
                                               const std::vector<std::string>& components, const std::string& outputVar)
{
    ...
    if (!result.sensitivities.empty()) {
        auto maxIt = std::max_element(
            result.sensitivities.begin(), result.sensitivities.end(),
            [](const auto& a, const auto& b) { return std::abs(a.sensitivity) < std::abs(b.sensitivity); });
...
MonteCarloEngine::MonteCarloEngine()
    : m_iterations(100), m_distribution("gaussian"), m_specMin(-std::numeric_limits<double>::infinity()),
...
double MonteCarloEngine::runSingleSimulation(const std::string& netlist, const std::map<std::string, double>& params)
{
...
MonteCarloResult MonteCarloEngine::run(const std::string& circuitFile)
{
    ...
    for (int i = 0; i < m_iterations; ++i) {
        ...
        if (i == 0 || output < result.min)
            result.min = output;
        if (i == 0 || output > result.max)
            result.max = output;
...
void CircuitOptimizer::addVariable(const std::string& name, double initial, double min, double max)
{
...
double CircuitOptimizer::evaluateObjective(const std::string& netlist, const std::map<std::string, double>& vars)
{
    ...
    for (int iter = 0; iter < 20; ++iter) {
        ...
        for (const auto& var : m_varNames) {
            ...
            for (double delta : {-stepSize, stepSize}) {
                double trial = std::clamp(newValues[var] + delta, m_varMin[0], m_varMax[0]);
                newValues[var] = trial;
...
WorstCaseAnalyzer::WorstCaseAnalyzer()
    : m_specMin(-std::numeric_limits<double>::infinity()), m_specMax(std::numeric_limits<double>::infinity())
{
}
...
double WorstCaseAnalyzer::runCorner(const std::string& netlist, const std::map<std::string, double>& values)
{
...
WorstCaseResult WorstCaseAnalyzer::analyze(const std::string& circuitFile)
{
    ...
    // Check margins
    result.margin = std::min(result.worstMin - m_specMin, m_specMax - result.worstMax);
...
void flux_stability_destroy(void* p)
{
    delete static_cast<StabilityAnalyzer*>(p);
}
...
void flux_sensitivity_destroy(void* p)
{
    delete static_cast<SensitivityAnalyzer*>(p);
}
...
void flux_monte_carlo_destroy(void* p)
{
    delete static_cast<MonteCarloEngine*>(p);
}
...
{
    return new CircuitOptimizer();
}
void flux_optimizer_destroy(void* p)
{
    delete static_cast<CircuitOptimizer*>(p);
}
...
{
    return new WorstCaseAnalyzer();
}
void flux_worst_case_destroy(void* p)
{
    delete static_cast<WorstCaseAnalyzer*>(p);
}
