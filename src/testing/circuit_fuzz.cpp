...
FuzzSummary CircuitFuzzer::runFuzzTest(const std::string& circuitId)
{
    ...
    summary.passRate = (summary.totalTests > 0) ? static_cast<double>(summary.passed) / summary.totalTests : 0.0;
...
std::map<std::string, double> CircuitFuzzer::mutateInputs(const std::map<std::string, double>& inputs) const
{
