...
CircuitValidationResult CrossSimulatorValidator::validateCircuit(const ValidationConfig& config,
                                                                 ...
                                                                 const std::string& reference_netlist)
{
    ...
    try {
        fluxscript_results =
            SimulationHelpers::runFluxScriptSimulation(fluxscript_netlist, config.simulation_time, config.time_step);
    ...
    if (reference_results.empty()) {
        try {
            if (config.reference_simulator == "ngspice") {
                reference_results = SimulationHelpers::runNgspiceSimulation(reference_netlist, config.simulation_time,
                                                                            config.time_step);
            } else {
    ...
    // Compare results
    result.comparisons = compareMeasurements(fluxscript_results, reference_results, config.absolute_tolerance,
    ...
    // Update statistics
    {
        ...
        if (result.overall_passed)
            m_passed_validations++;
    }

    return result;
}

std::vector<MeasurementComparison>
CrossSimulatorValidator::compareMeasurements(const std::map<std::string, double>& fluxscript_results,
                                             const std::map<std::string, double>& reference_results, double abs_tol,
                                             double rel_tol)
{
...
std::string CrossSimulatorValidator::generateReport(const CircuitValidationResult& result) const
{
    ...
    oss << std::left << std::setw(20) << "Name" << std::right << std::setw(12) << "FluxScript" << std::setw(12)
        << "Reference" << std::setw(12) << "Abs Error" << std::setw(12) << "Rel Error" << std::setw(10) << "Status"
        << "\n";
    oss << std::string(78, '-') << "\n";

    for (const auto& comp : result.comparisons) {
        oss << std::left << std::setw(20) << comp.name << std::right << std::setw(12) << std::scientific
            << std::setprecision(4) << comp.fluxscript_value << std::setw(12) << comp.reference_value << std::setw(12)
            << comp.absolute_error << std::setw(12) << comp.relative_error << std::setw(10) << comp.status << "\n";
    }
...
std::vector<CircuitValidationResult>
CrossSimulatorValidator::validateCircuitBatch(const std::vector<ValidationConfig>& configs,
                                              ...
                                              const std::vector<std::string>& reference_netlists)
{
    ...
    for (size_t i = 0; i < configs.size(); i++) {
        ...
        results.push_back(validateCircuit(configs[i], fluxscript_netlists[i], reference_netlists[i]));
    }
...
bool CrossSimulatorValidator::cacheReferenceResult(const std::string& circuit_key,
...
const std::map<std::string, double>* CrossSimulatorValidator::getCachedReference(const std::string& circuit_key) const
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
std::map<std::string, double> runFluxScriptSimulation(const std::string& netlist, double sim_time, double time_step)
{
...
std::map<std::string, double> runNgspiceSimulation(const std::string& netlist, double sim_time, double time_step)
{
...
std::map<std::string, double> extractMeasurements(const std::vector<double>& time, const std::vector<double>& voltage,
