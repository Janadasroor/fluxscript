...
bool SmartSignalItem::compileJITExpression(std::string* error)
{
    ...
    if (m_source_type != SignalSourceType::JITCompiled && m_source_type != SignalSourceType::Behavioral) {
        if (error)
            *error = "Signal source type is not JIT-compatible";
        return false;
    }

    if (m_params.jit_expression.empty()) {
        if (error)
            *error = "JIT expression is empty";
        return false;
    }

    auto& jit_manager = JITManager::instance();

    std::string component_source = "update(t, inputs, outputs, state) {\n"
                                   "    var result = " +
                                   m_params.jit_expression +
                                   ";\n"
                                   "    outputs[0] = result;\n"
                                   "    return result;\n"
                                   "}\n";
...
double SmartSignalItem::generateSine(double t)
{
    ...
    return m_params.dc_offset + m_params.amplitude * std::sin(angular_freq * (t - m_params.delay) + m_params.phase);
}
...
void SignalGroup::removeSignal(const std::string& sig_name)
{
    signals.erase(
        std::remove_if(signals.begin(), signals.end(),
                       [&sig_name](const std::shared_ptr<SmartSignalItem>& s) { return s->name() == sig_name; }),
        signals.end());
}
    ...
    for (const auto& signal : signals) {
        ...
        if (!result.success) {
            ...
            combined_result.error_message +=
                "Failed to generate " + signal->name() + ": " + result.error_message + "; ";
        }
    }
...
bool SmartSignalManager::addSignal(std::shared_ptr<SmartSignalItem> signal)
{
    ...
    if (!signal)
        return false;
    if (m_signals.count(signal->name()))
        return false;
