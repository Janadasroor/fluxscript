...
public:
    ...
    // Audit logging
    void logEvent(AuditEntry::EventType type, const std::string& source, const std::string& description,
                  bool success = true);
...
public:
    FluxBoundsException(size_t index, size_t size, const std::string& array)
        : FluxSecurityException("Array bounds violation: index " + std::to_string(index) + " out of bounds [0, " +
                                std::to_string(size) + ") for " + array)
    {
    }
};
