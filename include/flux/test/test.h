...
struct TestRunResult
{
    ...
    TestRunResult()
        : totalTests(0), totalPassed(0), totalFailed(0), totalSkipped(0), totalErrors(0), totalDurationMs(0.0),
          success(true)
    {
    }
...
public:
    ...
    AssertionFailure(const std::string& msg, const std::string& f, int l) : message(msg), file(f), line(l) {}
};
...
public:
    ...
    void setCurrentTest(const std::string& name, const std::string& suite, const std::string& file, int line)
    {
        m_currentTest = name;
...
inline void assertTrue(bool condition, const std::string& msg = "", const char* file = __FILE__, int line = __LINE__)
{
    if (!condition) {
...
inline void assertFalse(bool condition, const std::string& msg = "", const char* file = __FILE__, int line = __LINE__)
{
    if (condition) {
...
inline void assertEquals(double expected, double actual, double tolerance = 1e-10, const std::string& msg = "",
                         const char* file = __FILE__, int line = __LINE__)
{
    if (std::abs(expected - actual) > tolerance) {
        std::ostringstream oss;
        oss << "Expected " << formatValue(expected) << " but got " << formatValue(actual);
        if (!msg.empty())
            oss << ": " << msg;
...
inline void assertEquals(int expected, int actual, const std::string& msg = "", const char* file = __FILE__,
                         int line = __LINE__)
{
    if (expected != actual) {
        std::ostringstream oss;
...
template <typename T>
inline void assertEquals(const T& expected, const T& actual, const std::string& msg = "", const char* file = __FILE__,
                         int line = __LINE__)
{
    if (expected != actual) {
        std::ostringstream oss;
...
inline void assertNotEquals(double a, double b, double tolerance = 1e-10, const std::string& msg = "",
                            const char* file = __FILE__, int line = __LINE__)
{
...
inline void assertLessThan(double a, double b, const std::string& msg = "", const char* file = __FILE__,
                           int line = __LINE__)
{
    if (a >= b) {
        std::ostringstream oss;
...
inline void assertGreaterThan(double a, double b, const std::string& msg = "", const char* file = __FILE__,
                              int line = __LINE__)
{
    if (a <= b) {
        std::ostringstream oss;
...
inline void assertNear(double expected, double actual, double tolerance = 1e-6, const std::string& msg = "",
                       const char* file = __FILE__, int line = __LINE__)
{
    if (std::abs(expected - actual) > tolerance) {
        std::ostringstream oss;
        oss << "Expected " << formatValue(expected) << "  " << tolerance << " but got " << formatValue(actual);
        if (!msg.empty())
            oss << ": " << msg;
        throw AssertionFailure(oss.str(), file, line);
    }
...
inline void assertThrows(std::function<void()> func, const std::string& msg = "", const char* file = __FILE__,
                         int line = __LINE__)
{
    try {
        func();
...
struct TestRegistration
{
    ...
    TestRegistration(const std::string& n, const std::string& s, TestFunction f, const std::string& fpath, int l)
...
public:
    ...
    void registerTest(const std::string& name, const std::string& suite, TestFunction func, const std::string& file,
                      int line)
    {
...
{
public:
    static TestRunResult runAll() { return runFiltered(""); }
    ...
    static TestRunResult runFiltered(const std::string& filter)
    {
        ...
        for (const auto& test : registry.tests()) {
            ...
            if (!filter.empty()) {
                if (test.name.find(filter) == std::string::npos && test.suite.find(filter) == std::string::npos) {
                    continue;
                }
            }
    ...
    static TestResult runTest(const TestRegistration& test)
    {
        ...
        if (ctx.isVerbose()) {
            std::cout << "  [" << (result.status == TestStatus::Passed ? "" : "") << "] " << test.name << " ("
                      << std::fixed << std::setprecision(2) << result.durationMs << "ms)" << std::endl;
...
public:
    static void printSummary(const TestRunResult& result)
    {
        ...
        for (const auto& suite : result.suites) {
            ...
            std::cout << "  Time:    " << std::fixed << std::setprecision(2) << suite.totalDurationMs << "ms\n\n";
        ...
        std::cout << "\n";
        std::cout << "Total:  " << result.totalTests << " tests, " << result.totalPassed << " passed, "
                  << result.totalFailed << " failed, " << result.totalErrors << " errors\n";
        std::cout << "Time:   " << std::fixed << std::setprecision(2) << result.totalDurationMs << "ms\n";
...
#define FLUX_TEST(suite, name)                                                                                         \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            Flux::Test::TestRegistry::instance().registerTest(#name, #suite, flux_test_##suite##_##name, __FILE__,     \
                                                              __LINE__);                                               \
        }                                                                                                              \
...
#define FLUX_TEST_FIXTURE(suite, name, fixture) FLUX_TEST(suite, name)

// Assertion macros
#define FLUX_ASSERT_TRUE(cond) Flux::Test::Assert::assertTrue(cond, #cond, __FILE__, __LINE__)

#define FLUX_ASSERT_FALSE(cond) Flux::Test::Assert::assertFalse(cond, #cond, __FILE__, __LINE__)

#define FLUX_ASSERT_EQ(expected, actual) Flux::Test::Assert::assertEquals(expected, actual, "", __FILE__, __LINE__)

#define FLUX_ASSERT_NEAR(expected, actual, tol)                                                                        \
    Flux::Test::Assert::assertNear(expected, actual, tol, "", __FILE__, __LINE__)

#define FLUX_ASSERT_THROWS(expr) Flux::Test::Assert::assertThrows([&]() { expr; }, "", __FILE__, __LINE__)

#define FLUX_FAIL(msg) throw Flux::Test::AssertionFailure(msg, __FILE__, __LINE__)
