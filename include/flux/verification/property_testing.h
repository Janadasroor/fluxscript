...
public:
    ...
    generateCircuitParameters(const std::vector<std::string>& param_names,
                              const std::map<std::string, std::pair<double, double>>& ranges);

    // Mutate existing parameters (for shrinking)
    std::map<std::string, double> mutateParameters(const std::map<std::string, double>& base,
...
struct Property
{
    ...
    std::function<bool(const std::map<std::string, double>& params, const std::map<std::string, double>& result,
...
public:
    ...
    // Run property tests
    PropertyTestResult
    testProperty(const std::string& property_name, const PropertyTestConfig& config,
                 std::function<std::map<std::string, double>(const std::map<std::string, double>&)> simulate);

    // Test all registered properties
    std::vector<PropertyTestResult>
    testAllProperties(const PropertyTestConfig& config,
                      std::function<std::map<std::string, double>(const std::map<std::string, double>&)> simulate);
