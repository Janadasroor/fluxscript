...
struct DocConfig
{
    ...
    DocConfig()
        : outputFormat("markdown"), language("en"), includeBOM(true), includeSimulation(true),
          includeTestProcedures(true), includeTroubleshooting(true), includeSchematics(false), includeWaveforms(false)
    {
    }
};
...
Documentation generate_docs(const std::string& circuitFile, const DocConfig& config = DocConfig());
