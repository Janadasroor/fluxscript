...
struct BenchmarkResult
{
    ...
    BenchmarkResult()
        : meanTime(0), stdDev(0), minTime(0), maxTime(0), medianTime(0), iterations(0), opsPerSecond(0), speedup(1.0),
          passed(true)
    {
    }
};
...
public:
    ...
    void registerTest(const std::string& name, const std::string& language, const std::string& category,
                      BenchmarkFunc func, BenchmarkFunc validation = nullptr);

    // Run benchmarks
    BenchmarkResult runTest(const std::string& name, int iterations = 1000, int warmupIterations = 100);

    BenchmarkSuiteResult runAll(int iterations = 1000, int warmupIterations = 100);
    BenchmarkSuiteResult runByCategory(const std::string& category, int iterations = 1000);
    BenchmarkSuiteResult runByLanguage(const std::string& language, int iterations = 1000);
    ...
    static double calculateSpeedup(const BenchmarkResult& baseline, const BenchmarkResult& comparison);
    static std::string generateComparisonTable(const std::vector<BenchmarkResult>& results);
...
private:
    ...
    BenchmarkResult analyzeResults(const std::string& name, const std::string& language, const std::string& category,
                                   const std::vector<double>& times, int iterations);
};

// Convenience macros
#define FLUX_BENCHMARK(name, language, category) FLUX_BENCHMARK_CONCAT(name, language, category, __LINE__)

#define FLUX_BENCHMARK_CONCAT(name, language, category, line) FLUX_BENCHMARK_IMPL(name, language, category, line)
...
#define FLUX_BENCHMARK_IMPL(name, language, category, id)                                                              \
    ...
    {                                                                                                                  \
        flux_bench_##id##_reg()                                                                                        \
        {                                                                                                              \
            Flux::Benchmark::BenchmarkRunner::instance().registerTest(name, language, category, flux_bench_##id);      \
        }                                                                                                              \
