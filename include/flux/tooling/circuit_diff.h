...
struct CircuitDiffResult
{
    ...
    CircuitDiffResult()
        : addedComponents(0), removedComponents(0), modifiedComponents(0), addedNets(0), removedNets(0),
...
struct DiffOptions
{
    ...
    DiffOptions()
        : ignoreWhitespace(false), ignoreComments(false), ignoreOrder(true), showPerformanceImpact(true),
          showRecommendations(true), outputFormat("text"), contextLines(3)
    {
    }
};
...
private:
    ...
    std::vector<ComponentDiff> diffComponents(const std::string& oldCircuit, const std::string& newCircuit);
    std::vector<NetDiff> diffNets(const std::string& oldCircuit, const std::string& newCircuit);
