...
struct MemoryStats
{
    ...
    MemoryStats() : totalAllocated(0), totalFreed(0), currentUsage(0), peakUsage(0), allocationCount(0), leakCount(0) {}
};
...
// RAII timer macro
#define FLUX_PROFILE_FUNCTION() Flux::Profiler::Profiler::ScopedTimer _profiler_timer(__FUNCTION__, __FILE__, __LINE__)

#define FLUX_PROFILE_SCOPE(name) Flux::Profiler::Profiler::ScopedTimer _profiler_timer(name, __FILE__, __LINE__)

// Memory tracking macros
#ifdef FLUX_ENABLE_MEMORY_TRACKING
#define FLUX_TRACK_ALLOC(ptr, size) Flux::Profiler::Profiler::instance().trackAllocation(ptr, size, __FILE__, __LINE__)
#define FLUX_TRACK_FREE(ptr) Flux::Profiler::Profiler::instance().trackDeallocation(ptr)
#else
