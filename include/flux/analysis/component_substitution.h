...
public:
    ...
    std::vector<SubstitutionSuggestion> findSubstitutes(
        const ComponentSpec& original,
        const std::vector<std::string>& preferredManufacturers = {});
    
    // Find substitutes by part number
    std::vector<SubstitutionSuggestion> findSubstitutesByPartNumber(
    ...
    void setPreferredManufacturers(const std::vector<std::string>& mfrs) {
        m_preferredManufacturers = mfrs;
    }
...
private:
    ...
    std::vector<SubstitutionSuggestion> rankSuggestions(
        std::vector<SubstitutionSuggestion>& suggestions);
    void checkParametricCompatibility(const ComponentSpec& original,
                                       const ComponentSpec& substitute,
