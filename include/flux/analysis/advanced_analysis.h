...
struct StabilityResult
{
    ...
    StabilityResult()
        : gainMargin(0), phaseMargin(0), gainMarginFreq(0), phaseMarginFreq(0), bandwidth(0), peakGain(0),
...
struct MonteCarloResult
{
    ...
    MonteCarloResult() : iterations(0), mean(0), stdDev(0), min(0), max(0), yield(0), worstCase(0) {}
...
struct OptimizationResult
{
    ...
    OptimizationResult() : converged(false), iterations(0), objectiveValue(0), improvement(0) {}
...
struct WorstCaseResult
{
    ...
    WorstCaseResult() : nominal(0), worstMin(0), worstMax(0), margin(0), meetsSpec(true) {}
...
private:
    double findGainMargin(const std::vector<double>& freq, const std::vector<double>& phase);
    double findPhaseMargin(const std::vector<double>& freq, const std::vector<double>& gain);
    double find3dBBandwidth(const std::vector<double>& freq, const std::vector<double>& gain);
};
...
public:
    ...
    SensitivityResult analyze(const std::string& circuitFile, const std::vector<std::string>& components,
                              const std::string& outputVar);

private:
    double calculateSensitivity(const std::string& netlist, const std::string& paramName, double nominalValue,
                                double deltaPercent, const std::string& outputVar);
};
...
private:
    ...
    double runSingleSimulation(const std::string& netlist, const std::map<std::string, double>& params);
};
...
public:
    ...
    void addVariable(const std::string& name, double initial, double min, double max);
...
private:
    ...
    double evaluateObjective(const std::string& netlist, const std::map<std::string, double>& vars);
};
...
private:
    ...
    double runCorner(const std::string& netlist, const std::map<std::string, double>& values);
};
