...
struct Dual
{
    ...
    Dual operator+(const Dual& other) const { return Dual(value + other.value, dual + other.dual); }
    ...
    Dual operator*(const Dual& other) const
    {
        ...
        return Dual(value * other.value, dual * other.value + value * other.dual);
    }

    Dual operator/(const Dual& other) const
    {
        // Quotient rule: (u/v)' = (u'v - uv') / v
        return Dual(value / other.value, (dual * other.value - value * other.dual) / (other.value * other.value));
    }

    Dual operator-() const { return Dual(-value, -dual); }
...
inline Dual atan2(const Dual& y, const Dual& x)
{
    ...
    return Dual(std::atan2(y.value, x.value), (y.dual * x.value - x.dual * y.value) / denom);
}
...
inline Dual pow(const Dual& base, const Dual& exp)
{
    ...
    return Dual(powVal, powVal * (exp.dual * std::log(base.value) + exp.value * base.dual / base.value));
}
...
public:
    ...
    static double derivativeWithSeed(const std::function<double(double)>& f, double x, double seed = 1.0)
    {
        const double h = 1e-8;
    ...
    static std::vector<double> gradient(const std::function<double(const std::vector<double>&)>& f,
    ...
    jacobian(const std::function<std::vector<double>(const std::vector<double>&)>& f, const std::vector<double>& x)
    {
        const double h = 1e-8;
        size_t n = x.size();
    ...
    static Dual evalDual(const std::function<Dual(const Dual&)>& f, const Dual& x) { return f(x); }
};
...
public:
    ...
    Var(double value = 0.0) { node = std::make_shared<Node>(value); }

    double value() const { return node->value; }
    double grad() const { return node->gradient; }

    void zeroGrad() { node->gradient = 0.0; }
    ...
    std::shared_ptr<Var> operator/(const std::shared_ptr<Var>& other) const
    {
        ...
        result->node->backwardFunc = [this, other, result]() {
            node->gradient += result->node->gradient / other->node->value;
            other->node->gradient -= result->node->gradient * node->value / (other->node->value * other->node->value);
        };
        return result;
    }
    ...
    std::shared_ptr<Var> operator-() const
    {
        ...
        result->node->backwardFunc = [this, result]() { node->gradient -= result->node->gradient; };
        return result;
    }
    ...
    static std::shared_ptr<Var> log(const std::shared_ptr<Var>& x)
    {
        ...
        result->node->inputs = {x->node};
        result->node->backwardFunc = [x, result]() { x->node->gradient += result->node->gradient / x->node->value; };
        return result;
    }
    ...
    static std::shared_ptr<Var> pow(const std::shared_ptr<Var>& x, double exp)
    {
        ...
        result->node->backwardFunc = [x, result, exp]() {
            x->node->gradient += result->node->gradient * exp * std::pow(x->node->value, exp - 1);
        };
        return result;
    }
...
public:
    ...
    // Compute gradient
    static std::vector<double> gradient(const std::shared_ptr<Var>& output,
