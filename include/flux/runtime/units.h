...
struct UnitDimensions {
    ...
    UnitDimensions pow(int exponent) const {
        return {
            static_cast<int8_t>(mass * exponent),
            static_cast<int8_t>(length * exponent),
            static_cast<int8_t>(time * exponent),
            static_cast<int8_t>(current * exponent),
            static_cast<int8_t>(temperature * exponent),
            static_cast<int8_t>(amount * exponent),
...
public:
    ...
    QuantifiedValue(double value, const UnitDimensions& dims)
        : m_value(value), m_unit(dims, 1.0, "", "") {}
    QuantifiedValue(double value, const ScaledUnit& unit)
...
namespace Convert {
    ...
    // Common conversions
    inline double volts(double v) { return v; }
    inline double millivolts(double mv) { return mv * 0.001; }
    inline double microvolts(double uv) { return uv * 1e-6; }
    inline double kilovolts(double kv) { return kv * 1000.0; }
    
    inline double ohms(double r) { return r; }
    inline double milliohms(double mr) { return mr * 0.001; }
    inline double kiloohms(double kr) { return kr * 1000.0; }
    inline double megaohms(double mr) { return mr * 1e6; }
    
    inline double farads(double f) { return f; }
    inline double millifarads(double mf) { return mf * 0.001; }
    inline double microfarads(double uf) { return uf * 1e-6; }
    inline double nanofarads(double nf) { return nf * 1e-9; }
    inline double picofarads(double pf) { return pf * 1e-12; }
    
    inline double henrys(double h) { return h; }
    inline double millihenrys(double mh) { return mh * 0.001; }
    inline double microhenrys(double uh) { return uh * 1e-6; }
    
    inline double amperes(double a) { return a; }
    inline double milliamperes(double ma) { return ma * 0.001; }
    inline double microamperes(double ua) { return ua * 1e-6; }
    
    inline double hertz(double hz) { return hz; }
    inline double kilohertz(double khz) { return khz * 1000.0; }
    inline double megahertz(double mhz) { return mhz * 1e6; }
    inline double gigahertz(double ghz) { return ghz * 1e9; }
    
    inline double seconds(double s) { return s; }
    inline double milliseconds(double ms) { return ms * 0.001; }
    inline double microseconds(double us) { return us * 1e-6; }
    inline double nanoseconds(double ns) { return ns * 1e-9; }
}
...
public:
    explicit UnitParseException(const std::string& unit)
        : UnitException("Cannot parse unit: " + unit) {}
};
