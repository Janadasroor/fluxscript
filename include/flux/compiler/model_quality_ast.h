...
public:
    GoldenDeclAST(const std::string& name, const std::string& filename)
        : Name(name), Filename(filename) {}
    
    TypedValue codegen(CodegenContext& context) override;
    
    void loadWaveform();
    void addPoint(double t, double v) { TimePoints.push_back(t); Values.push_back(v); }
...
public:
    ConvergeDeclAST(const std::string& node, int maxIter, double eps)
        : Node(node), MaxIterations(maxIter), Epsilon(eps) {}
...
public:
    DiscontinuityDeclAST(const std::string& node, double threshold)
...
public:
    StateDeclAST(const std::string& node, int depth)
        : Node(node), HistoryDepth(depth) {}
...
public:
    ToleranceDeclAST(double absTol, double relTol)
        : AbsoluteTolerance(absTol), RelativeTolerancePercent(relTol) {}
