...
public:
    SubcktInstanceAST(const std::string& name, const std::string& subckt) : InstanceName(name), SubcktName(subckt) {}
...
public:
    ...
    void addContributor(std::unique_ptr<ExprAST> contributor) { Contributors.push_back(std::move(contributor)); }
    void setTolerance(const std::string& param, const std::string& value) { Tolerances[param] = value; }
...
public:
    VoltageAccessAST(const std::string& nodeP, const std::string& nodeN = "0") : NodeP(nodeP), NodeN(nodeN) {}
...
public:
    PinMapAST(const std::string& symbol, const std::string& subckt) : SymbolName(symbol), SubcktName(subckt) {}

    TypedValue codegen(CodegenContext& context) override;

    void addMapping(const std::string& symbolPin, const std::string& subcktPin) { Mappings[symbolPin] = subcktPin; }
