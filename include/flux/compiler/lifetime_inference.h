...
private:
    ...
    void collectFromExpr(const ExprAST* expr, const std::set<std::string>& lifetimeParamNames);

    // Collect constraints from type annotations (recursive)
    void collectFromType(const FluxType& type, const std::string& containingLifetime,
    ...
    bool checkConsistency(const std::set<std::string>& lifetimeParamNames,
                          ...
                          const std::vector<std::pair<std::string, FluxType>>& args, const FluxType& returnType);
};
