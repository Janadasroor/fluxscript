...
public:
    OutputsExprAST(const std::string& node, std::unique_ptr<ExprAST> val) : NodeName(node), Value(std::move(val)) {}
    TypedValue codegen(CodegenContext& context) override;
};
...
public:
    InitialCondAST(const std::string& node, std::unique_ptr<ExprAST> val) : Node(node), Value(std::move(val)) {}
    TypedValue codegen(CodegenContext& context) override;
};
...
public:
    TransientAnalysisAST(double step, double stop) : Timestep(step), StopTime(stop), StartTime(0.0), MaxTimestep(0.0) {}
