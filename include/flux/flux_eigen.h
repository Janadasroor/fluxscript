...
public:
    ...
    void registerMatrix(double* data, int rows, int cols) { m_matrices[data] = {rows, cols, data}; }

    bool isMatrix(double* data) const { return m_matrices.find(data) != m_matrices.end(); }
    ...
    void unregisterMatrix(double* data) { m_matrices.erase(data); }
