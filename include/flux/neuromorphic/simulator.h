...
struct NeuronState
{
    ...
    NeuronState() : membrane_potential(-70), recovery_variable(0), conductance(0), last_spike_time(-1), spike_count(0)
    {
    }
};
...
struct LIFParameters
{
    ...
    LIFParameters() : tau_m(20), tau_ref(2), v_rest(-70), v_thresh(-55), v_reset(-75), i_bias(0) {}
};
...
struct HHParameters
{
    ...
    HHParameters() : C_m(1.0), g_Na(120), g_K(36), g_L(0.3), E_Na(50), E_K(-77), E_L(-54.4) {}
};
...
struct IzhikevichParameters
{
    ...
    IzhikevichParameters() : a(0.02), b(0.2), c(-65), d(8), v_rest(-65), v_thresh(30) {}
...
struct SynapseParameters
{
    ...
    SynapseParameters() : tau_rise(0.5), tau_decay(3.0), g_max(1.0), u(0.5), tau_facil(0), tau_recov(800) {}
};
...
struct PlasticityParameters
{
    ...
    PlasticityParameters()
        : rule(PlasticityRule::None), tau_plus(20), tau_minus(20), A_plus(0.005), A_minus(0.0052), eta(0.01)
    {
    }
};
...
public:
    ...
    // Configuration
    void setParameters(double tauPlus, double tauMinus, double APlus, double AMinus);

    // Weight update
    double computeWeightChange(double preTime, double postTime, double currentWeight);
